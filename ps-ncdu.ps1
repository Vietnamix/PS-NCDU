        ...
        try {
            ...
            foreach ($d in $odDirs) {
                if (-not ($paths | Where-Object { $_.Path -eq $d.FullName })) {
                    $type = if ($d.Name -match "Enterprise|Business|Corp") { "OneDrive Entreprise" } else { "OneDrive" }
...
function bColor(sz){
    ...
    if(sz>=10485760)return 'var(--warn2)';
    return 'var(--ok)';
}
...
function renderContent(path){
    ...
    if(!scan){
        if(isDDir){
            ...
            document.getElementById('mainContent').innerHTML='<div class="msg"><span class="ic">&#128274;</span><h3>Dossier protege par ACL NTFS</h3><p>Inaccessible avec ce compte.</p><p class="hint-d">Demande via <strong>xxxxxxxxx</strong></p><code>'+escH(path)+'</code></div>';
        } else if(isUDir){
...
<head>
    ...
    <style>
    :root{--bg:#f5f7fa;--surface:#ffffff;--card:#eef2f7;--card-h:#e3e9f1;--border:#d5dde7;--border-l:#e6ebf2;--text:#16202e;--tm:#54627a;--td:#95a3b6;--accent:#51607a;--al:#3f4b5f;--ag:rgba(81,96,122,.09);--ok:#1f7a5c;--warn:#b5731a;--warn2:#cf8f2e;--danger:#c0392b;--unscanned:#6b4ca3;--od:#51607a;--shadow:0 1px 3px rgba(16,32,54,.07),0 4px 12px rgba(16,32,54,.05);--r:8px;--rs:5px;--rp:20px}
    [data-theme="dark"]{--bg:#0e151f;--surface:#151e2b;--card:#1c2738;--card-h:#243245;--border:#2a3a50;--border-l:#1f2c3e;--text:#e2e8f1;--tm:#9aa8bd;--td:#637186;--accent:#7e899d;--al:#9aa3b2;--ag:rgba(126,137,157,.16);--ok:#34c08a;--warn:#e0a23c;--warn2:#e6b052;--danger:#e35d52;--unscanned:#9d83d6;--od:#8993a5;--shadow:0 2px 14px rgba(0,0,0,.45)}
    *{box-sizing:border-box;margin:0;padding:0}
    ...
    .header-outer{background:linear-gradient(135deg,#2c3442 0%,#4d5b74 55%,#394355 100%);position:sticky;top:0;z-index:200;box-shadow:0 2px 12px rgba(20,28,40,.22)}
    [data-theme="dark"] .header-outer{background:linear-gradient(135deg,#0e151f 0%,#1c2738 100%)}
    ...
    [data-theme="dark"] .hdr-chip{background:rgba(255,255,255,.1);color:var(--text);border-color:rgba(255,255,255,.25)}
    ...
    [data-theme="dark"] .hdr-input{background:rgba(255,255,255,.08);border-color:rgba(255,255,255,.2);color:var(--text)}
    [data-theme="dark"] .hdr-input:focus{background:rgba(255,255,255,.15);border-color:rgba(255,255,255,.45)}
    ...
    [data-theme="dark"] .hdr-btn{background:rgba(255,255,255,.1);color:var(--text);border-color:rgba(255,255,255,.25)}
    [data-theme="dark"] .hdr-btn:hover{background:rgba(255,255,255,.2)}
    ...
    [data-theme="dark"] .hdr-av{background:rgba(255,255,255,.2);border-color:rgba(255,255,255,.35);color:var(--text)}
    ...
    [data-theme="dark"] .sp{background:rgba(255,255,255,.08);color:var(--tm)}[data-theme="dark"] .sp b{color:var(--text)}
    .sp-od{background:rgba(81,96,122,.25);border:1px solid rgba(81,96,122,.5)}
    [data-theme="dark"] .sp-od{background:rgba(137,147,165,.15);border-color:rgba(137,147,165,.4)}
    .vdg{width:1px;height:14px;background:rgba(255,255,255,.2);flex-shrink:0}
    [data-theme="dark"] .vdg{background:rgba(255,255,255,.2)}
    ...
    [data-theme="dark"] .bcp{background:rgba(255,255,255,.05)}
    ...
    [data-theme="dark"] .bcp-a{color:var(--tm)}[data-theme="dark"] .bcp-a:hover,[data-theme="dark"] .bcp-a.c{background:rgba(255,255,255,.15);color:var(--text)}
    ...
    .alert-det code{padding:1px 5px;border-radius:3px;background:rgba(192,57,43,.08);color:var(--danger)}
    ...
    tbody tr.row-ex{background:rgba(181,115,26,.05)}tbody tr.row-ex:hover{background:rgba(181,115,26,.1)}
    tbody tr.row-un{background:rgba(107,76,163,.05)}tbody tr.row-un:hover{background:rgba(107,76,163,.1)}
    tbody tr.row-dn{background:rgba(192,57,43,.05)}tbody tr.row-dn:hover{background:rgba(192,57,43,.1)}
    ...
    .tb-dir{background:rgba(81,96,122,.1);color:var(--al)}.tb-file{background:var(--card);color:var(--td)}
    .tb-ex{background:rgba(181,115,26,.12);color:var(--warn)}.tb-un{background:rgba(107,76,163,.12);color:var(--unscanned)}.tb-dn{background:rgba(192,57,43,.12);color:var(--danger)}
    .ex-lbl{font-size:.68em;background:rgba(181,115,26,.12);color:var(--warn);padding:2px 6px;border-radius:3px;margin-left:5px;font-weight:500}
    .dn-lbl{font-size:.68em;background:rgba(192,57,43,.12);color:var(--danger);padding:2px 6px;border-radius:3px;margin-left:5px;font-weight:500}
    ...
    .bar-ex{height:7px;background:repeating-linear-gradient(45deg,rgba(181,115,26,.1),rgba(181,115,26,.1) 4px,rgba(181,115,26,.2) 4px,rgba(181,115,26,.2) 8px)}
    .bar-uns{height:7px;background:repeating-linear-gradient(45deg,rgba(107,76,163,.1),rgba(107,76,163,.1) 4px,rgba(107,76,163,.2) 4px,rgba(107,76,163,.2) 8px)}
    .bar-dn{height:7px;background:repeating-linear-gradient(45deg,rgba(192,57,43,.1),rgba(192,57,43,.1) 4px,rgba(192,57,43,.2) 4px,rgba(192,57,43,.2) 8px)}
    ...
    .msg .hint{margin-top:12px;font-size:.78em;background:rgba(181,115,26,.1);color:var(--warn);border:1px solid rgba(181,115,26,.3);padding:7px 14px;border-radius:var(--rs);display:inline-block}
    .msg .hint-u{margin-top:12px;font-size:.78em;background:rgba(107,76,163,.1);color:var(--unscanned);border:1px solid rgba(107,76,163,.3);padding:7px 14px;border-radius:var(--rs);display:inline-block}
    .msg .hint-d{margin-top:12px;font-size:.78em;background:rgba(192,57,43,.1);color:var(--danger);border:1px solid rgba(192,57,43,.3);padding:7px 14px;border-radius:var(--rs);display:inline-block}
